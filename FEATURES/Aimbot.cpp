...
bool CAimbot::good_backtrack_tick(int tick)
{
	...
	float correct = clamp(nci->GetLatency(FLOW_OUTGOING) + LerpTime(), 0.f, 2.f);
...
Vector CAimbot::multipoint(SDK::CBaseEntity* entity, int option)
{
	...
	case 2:
	{
		int hitboxes[] = { SDK::HitboxList::HITBOX_HEAD, SDK::HitboxList::HITBOX_BELLY, SDK::HitboxList::HITBOX_LEFT_THIGH, SDK::HitboxList::HITBOX_RIGHT_THIGH, SDK::HitboxList::HITBOX_LEFT_CALF, SDK::HitboxList::HITBOX_RIGHT_CALF, SDK::HitboxList::HITBOX_LEFT_FOOT, SDK::HitboxList::HITBOX_RIGHT_FOOT };
		for (int i = 0; i < ARRAYSIZE(hitboxes); i++)
		{
			for (auto point : GetMultiplePointsForHitbox(local_player, entity, hitboxes[i], matrix))
			{
	...
	if (SETTINGS::settings.hsinair == 0)
	{
	...
	else if (SETTINGS::settings.hsinair == 1)
	{
		if (entity->GetFlags() & !FL_ONGROUND)
		{
			int hitboxes[] = { SDK::HitboxList::HITBOX_PELVIS, SDK::HitboxList::HITBOX_UPPER_CHEST, SDK::HitboxList::HITBOX_LOWER_CHEST };
		}
	}
