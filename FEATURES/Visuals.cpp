...
void CVisuals::Draw()
{
	...
	for (int i = 1; i <= 65; i++)
	{
		...
		if (!is_teammate)
		{
			if (SETTINGS::settings.fov_bool && !SETTINGS::settings.autorevolver_bool) DrawFovArrows(entity, CColor(arrow_colour[0], arrow_colour[1], arrow_colour[2], alpha));
		}
		else if (is_teammate)
		{
			if (SETTINGS::settings.arrowteam && !SETTINGS::settings.autorevolver_bool) DrawFovArrows(entity, CColor(arrowteam_colour[0], arrowteam_colour[1], arrowteam_colour[2], alpha));
		}
	}
}
...
{
	if (SETTINGS::settings.spread_bool && !SETTINGS::settings.autorevolver_bool) DrawInaccuracy();
...
void CVisuals::DrawBox(SDK::CBaseEntity* entity, CColor color, Vector pos, Vector top)
{
	...
	RENDER::DrawOutlineBox(pos.x - width / 2, top.y, width, height, color);
}
...
void CVisuals::DrawWeapon(SDK::CBaseEntity* entity, CColor color, int index, Vector pos, Vector top)
{
	...
	switch (SETTINGS::settings.weap_bool)
	{
	case 0:
		break;
	case 1:
	...
	case 2:
	{
		...
		if (entity->GetVelocity().Length2D() < 35)
		{
			int height = (pos.y - top.y), width = height / 2, distanceOn = distanceThing ? pos.y + 17 : pos.y + 8, distanceOn2 = distanceThing2 ? pos.y + 17 : pos.y + 8;
...
void CVisuals::DrawInfo(SDK::CBaseEntity* entity, CColor color, CColor alt, Vector pos, Vector top)
{
	...
	if (SETTINGS::settings.info_bool && !is_teammate)
	{
		...
		if (resolver->GetPlayerResolveInfo(entity).is_balance_adjust_triggered)
			stored_info.push_back(std::pair<std::string, CColor>("Resolve", GREEN));
	}
...
void CVisuals::DrawIndicator()
{
	...
	if (SETTINGS::settings.overrideenable)
	{
		...
		if (SETTINGS::settings.overridemethod == 0)
			RENDER::DrawF(10, screen_height - iY, FONTS::visuals_lby_font, false, false, SETTINGS::settings.overridething ? CColor(0, 255, 0) : CColor(255, 0, 0), " ");
		else if (SETTINGS::settings.overridemethod == 1)
		{
			GetAsyncKeyState(UTILS::INPUT::input_handler.keyBindings(SETTINGS::settings.overridekey)) ?
				RENDER::DrawF(10, screen_height - iY, FONTS::visuals_lby_font, false, false, CColor(0, 255, 0), " ") :
				RENDER::DrawF(10, screen_height - iY, FONTS::visuals_lby_font, false, false, CColor(255, 0, 0), " ");
		}
	}
	...
	{
		iY += 22;
		RENDER::DrawF(1500, screen_height - iY, FONTS::visuals_lby_font, false, false, CColor((1.f - breaking_lby_fraction) * 255.f, breaking_lby_fraction * 255.f, 0), "LBY");
	}
	if (SETTINGS::settings.rifk_arrow && !SETTINGS::settings.autorevolver_bool)
	{
...
void CVisuals::ColorConvertHSVtoRGB(float h, float s, float v, float& out_r, float& out_g, float& out_b)
{
	if (s == 0.0f)
	{
		// gray
...
void CVisuals::FreestandingIndicator()
{
	...
	{
		if (SETTINGS::settings.a_mode == 0 || SETTINGS::settings.aa_mode == 9)
		{
			...
			{
				RENDER::DrawF(cW, H * 0.80, FONTS::visuals_esp_font1, true, false, BLUE, "Auto");
				sprintf_s(bufferlineFakeAngle, " ", GLOBAL::lineFakeAngle);
				RENDER::DrawF(cW, H * 0.80 + 25, FONTS::visuals_esp_font1, true, false, BLUE, bufferlineFakeAngle);
			}
...
void CVisuals::GameSense()
{
	...
	if (GetTickCount() > lastTime)
	{
		kek++;
		if (kek > 10) {
			setClanTag("[Elevate.win]", "Elevate.win");
